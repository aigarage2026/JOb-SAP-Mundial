...
ARQUIVO_HISTORICO = "historico_vagas.txt"
...
def buscar_e_enviar():
        ...
        try:
            ...
            for _, job in jobs.iterrows():
                ...
                if link in historico:
                    print(f"⏭️ Já enviada anteriormente: {job.get('title')}")
                    continue
                ...
                print(f"🧐 Analisando nova vaga: {titulo} na {empresa}...")
                ...
                if analise_ia_vaga(titulo, descricao_vaga):
                    ...
                    novos_links_historico.append(link)
                else:
                    print(f"  ❌ REPROVADA (Kill Switch).")
    ...
    if vagas_aprovadas:
        ...
        html += "<p style='color: gray;'>Filtros: Sem restrição de cidadania US, 100% Remoto, Novas Vagas Apenas.</p><hr>"
        html += "".join([
        ...
        try:
            resend.Emails.send({
                ...
                "to": DESTINATARIO,
                "subject": "🎯 Novas Vagas SAP (Filtro Anti-Repetição)",
                "html": html
            })
            print(f"✅ E-mail enviado com sucesso!")
            # Só salvamos no histórico se o e-mail for enviado com sucesso
            salvar_no_historico(novos_links_historico)
        except Exception as e:
            print(f"❌ ERRO NO RESEND: {e}")
    else:
        print("ℹ️ Nenhuma vaga nova compatível encontrada hoje.")
